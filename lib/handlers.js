...
var dns = require("dns");
var { _performance, PerformanceObserver } = require("perf_hooks").performance;
...
handlers.index = function (data, callback) {
  // Reject a request that isnt a GET
  if (data.method == "get") {
  ...
  if (data.method == "get") {
    ...
    if (trimmedAssetName.length > 0) {
      ...
      helpers.getStaticAsset(trimmedAssetName, function (err, data) {
        if (!err && data) {
          //Determine the content type (Default to plain text otherwise)
          ...
          // console.log(
          //   "This is my error: " + err,
          //   "This is my content type " + contentType
          // );
          callback("Could not find file" + 404);
        }
      });
    }
  } else {
                    ...
                    _data.delete("checks", checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered while attempting to delete all of the user's checks. All checks may have not deleted from the system successfully.",
                          });
                        }
                      }
                    });
    ...
    _data.read("users", phone, function (err, userData) {
      ...
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          //1000 milliseconds, 60 secs, 60 minutes = token expiration is an hour
...
handlers._tokens.get = function (data, callback) {
  ...
  if (id) {
    ...
    callback(400, { Error: "Missing required field or ID is incorrect" });
  }
};
...
// Checks - put
//Required data - Id
// Optional data protocol, url, method, successCodes,timeoutSeconds (one needs to be updated)
  ...
  if (id) {
    ...
    _data.read("checks", id, function (err, checkData) {
      if (!err && checkData) {
                ...
                _data.read("users", checkData.userPhone, function (
                  ...
                  if (!err && userData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      callback(500, {
                        Error:
                          "Could not find the check on the user's object, thus could not delete check",
                      });
                    }
                  } else {
                    callback(500, {
                      Error:
                        "Could not find user who created check, thus could not remove the check from user",
                    });
                  }
                });
              } else {
        ...
        callback(400, { Error: "The specified check ID does nhot exist" });
      }
    });
  } else {
    callback(400, { Error: "Missing required field" });
